function [Aircraft] = EvalDescentVLM(Aircraft)
%
...
%     fprintf(1, "Descent Iteration %2d:\n", iter);
    ...
    for i = 1:length(L)
      ...
      conditions.V = TAS(i); % (m/s) or conditions.Mach = 0.6
      conditions.L = L(i); % (N) Lift at current condition
      conditions.T = 0.7*DGuess; % (N) Thrust approximation at current condition, prefer to use value from drag at previous point, which will be close enough

function [Aircraft] = EvalCruiseVLM(Aircraft)
%
...
%     fprintf(1, "Cruise Iteration %2d:\n", iter);
    ...
    DGuess = D(1);
    for i = 1:length(L)
      conditions.alpha = alphaGuess; % (deg) guess of alpha for newton solver initialization, the closer the better... (maybe even use the slope from last step)
      conditions.alt = Alt(i); % (m)
      conditions.V = TAS(i); % (m/s)
      conditions.L = L(i); % (N) Lift at current condition
      conditions.T = DGuess; % (N) Thrust approximation at current condition, prefer to use value from drag at previous point, which will be close enough
      ...
      D(i) = results.D-0.7*DGuess;
      DGuess = results.D;
      if i < length(L)
        alphaGuess = alphaGuess + (L(i+1)-L(i))/results.dLda; % basically the first stage of the newton solver, saves iterations...
      end
    end

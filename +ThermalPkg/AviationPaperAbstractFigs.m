...
Settings.Coolant.FuelPumpSink = 500;
Settings.Coolant.AmbientPumpReturn = 273 - 100;
Settings.Coolant.AmbientPumpSink = 273 + 300;
Settings.Coolant.Fuel = 250;
Settings.Coolant.Ambient = 273 - 40;
...
for ii = 1:length(powers)
    ThermalSystemP.Propulsion.MaxPower = ...
        powers(ii) * ones(length(ThermalSystemP.CompNames),1); % Fake numbers for now
    ...
    ThermalSystemA.Propulsion.MaxPower = ...
        powers(ii) * ones(length(ThermalSystemA.CompNames),1); % Fake numbers for now
    % Size the mass flow internal to the connections between the components
...
figure(3)
plot(powers,activemass(1,:),'linewidth',2,'color',RegalBlue)
hold on
plot(powers,activemass(2,:),'linewidth',2,'color',JungleGreen)
plot(powers,passivemass,'linewidth',2,'color',GuardsmanRed)
grid on

ylabel("Loop Mass Flow (kg/s)")
xlabel("Heat Generated by Source (W)")

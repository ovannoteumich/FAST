...
while (iter < MaxIter)
    ...
    % update the wing sizing, oversimplified version that is close enough?
    Sold = Aircraft.Geometry.Surfaces{Aircraft.Geometry.AreaSurf}.Span*mean(Aircraft.Geometry.Surfaces{Aircraft.Geometry.AreaSurf}.Chord)*2;
    Aircraft.Geometry.Surfaces{Aircraft.Geometry.AreaSurf}.Span = Aircraft.Geometry.Surfaces{Aircraft.Geometry.AreaSurf}.Span*sqrt(Aircraft.Specs.Aero.S/Sold);
    Aircraft.Geometry.Surfaces{Aircraft.Geometry.AreaSurf}.Chord = Aircraft.Geometry.Surfaces{Aircraft.Geometry.AreaSurf}.Chord*sqrt(Aircraft.Specs.Aero.S/Sold);

    Aircraft.Geometry = VLMPkg.MeshGeneration(Aircraft.Geometry,4); % Make smaller at the O(n^3) cost of your runtime :)
    ...
    conditions.alt = Aircraft.Specs.Performance.Alts.Crs; % (m)
    conditions.Mach = Aircraft.Specs.Performance.Vels.Crs; % (m/s)
    conditions.L = 9.81*(Aircraft.Specs.Weight.MTOW - 0.5*Aircraft.Specs.Weight.Fuel); % (N) Lift at cruise at half fuel, good baseline
    conditions.T = 10000; % (N) Thrust guess at current condition, prefer to use value from drag at previous point, which will be close enough
    conditions.TrimSurf = 2; % Surface Number used for trimming moment
    conditions.CG = [16,0.3,0]; % X,Y,Z coords (m) of the CG for stability and moment calcs
    conditions.Stab = true; % Stability calculations or not?  Creating variable for false is unnecessary
  
    results = VLMPkg.AeroSolver(Aircraft.Geometry,conditions);
    Aircraft.Specs.Stability.SMp = results.SM_Pitch;
    Aircraft.Specs.Stability.SMy = results.SM_Yaw;

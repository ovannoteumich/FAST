...
elseif LamSLS ~= 0
    ...
    splitPC = Aircraft.Specs.Power.PC.Split;

    % get alt range from ac struct
    altPC = Aircraft.Specs.Power.PC.Alt;

    % get indeces for which type
    iPC = Aircraft.Specs.Power.PC.Index;

    % make tko PC = 100%
    tkoEnd = Aircraft.Mission.Profile.SegEnd(1);
 

    for j = 1:length(splitPC)

function geom = MeshGeneration(geom, BaseSize)
  ...
  [rc_nose, yc_nose] = GetFuselageState(x_nose,geom,NoseLen,TubeLen);
  for i = 1:NoseSegs-1
  ...
  [rc_tube, yc_tube] = GetFuselageState(x_tube(2:end-1),geom,NoseLen,TubeLen);
  for i = 1:TubeSegs-2
  ...
  [rc_tail, yc_tail] = GetFuselageState(x_tail(1:end-1),geom,NoseLen,TubeLen);
  for i = 1:TailSegs-1
  ...
  for i = 1:NVTail
    ...
    if i == 1 % Root Stitch
      [rc_f, yc_f] = GetFuselageState(xc_v,geom,NoseLen,TubeLen);
  ...
  geom.Mesh.Node = [geom.Mesh.Node; xc_v(2:end-1), yc_v(2:end-1), zeros(size(xc_v(2:end-1)))];
  nN = length(geom.Mesh.Node);
  geom.Mesh.Elem = [geom.Mesh.Elem; (nN:-1:(nN-length(xc_v_base)+4))', (nN:-1:(nN-length(xc_v_base)+4))'-1, (nN:-1:(nN-length(xc_v_base)+4))'-length(xc_v_base), (nN:-1:(nN-length(xc_v_base)+4))'-length(xc_v_base)+1];
  geom.Mesh.Elem = [geom.Mesh.Elem; nN,nN-length(xc_v_base)+1,nN-length(xc_v_base)+2,nN-length(xc_v_base)+2];
  geom.Mesh.Elem = [geom.Mesh.Elem; nN-length(xc_v_base)+3,nN-2*length(xc_v_base)+4,nN-2*length(xc_v_base)+3,nN-2*length(xc_v_base)+3];
  ...
  for idx_surf = 1:length(geom.Surfaces)
    ...
    NumSpanPts = 1+ceil(max(diff(s_ctrl)).*surf.Span*1.5/BaseSize)*(surf.NumCtrlPts-1);
  ...
  for idx_surf = 1:length(geom.Surfaces)
    ...
    for i = 1:size(Surf_TE, 1)
      ...
      for w = 1:20
        wx = mid(1) + w*BaseSize*10; [rf, yf] = GetFuselageState(wx,geom,NoseLen,TubeLen);
...
function [rc, yc] = GetFuselageState(xq,geom,NoseLen,TubeLen)
  rc = zeros(size(xq)); yc = zeros(size(xq));
  idx_nose = xq <= NoseLen;
  if any(idx_nose)
    x_ns = [0 geom.Fuselage.Nose.XPos*geom.Fuselage.Diameter/2] + NoseLen;
    if ~isfield(geom.Fuselage.Nose, "Spline"); geom.Fuselage.Nose.Spline = 'linear'; end
    if ~isfield(geom.Fuselage.Nose, "YSpline"); geom.Fuselage.Nose.YSpline = geom.Fuselage.Nose.Spline; end
    if ~isfield(geom.Fuselage.Nose, "RSpline"); geom.Fuselage.Nose.RSpline = geom.Fuselage.Nose.Spline; end
    yc(idx_nose) = interp1(x_ns, [0 geom.Fuselage.Nose.YPos]*geom.Fuselage.Diameter/2, xq(idx_nose), geom.Fuselage.Nose.YSpline, 'extrap');
    rc(idx_nose) = interp1(x_ns, [1 geom.Fuselage.Nose.Radii]*geom.Fuselage.Diameter/2, xq(idx_nose), geom.Fuselage.Nose.RSpline, 'extrap');
  end
  ...
  if any(idx_tail)
    ...
    if ~isfield(geom.Fuselage.Tail, "Spline"); geom.Fuselage.Tail.Spline = 'linear'; end
    if ~isfield(geom.Fuselage.Tail, "YSpline"); geom.Fuselage.Tail.YSpline = geom.Fuselage.Tail.Spline; end
    if ~isfield(geom.Fuselage.Tail, "RSpline"); geom.Fuselage.Tail.RSpline = geom.Fuselage.Tail.Spline; end
...
%         [rf, yf] = GetFuselageState(xc_s,geom,NoseLen,TubeLen);
